 ...
 * 5. 执行提案
 * 
 * 运行环境：本地区块链节点 (hardhat node)
 * 
 * 使用方法：
 * 1. 启动本地节点: npx hardhat node
 * 2. 运行脚本: npx hardhat run scripts/storage-governance-example.js --network localhost
 */

const { ethers } = require("hardhat");

// 时间常量（与合约保持一致）
const VOTING_DELAY = 1 * 24 * 60 * 60;  // 1 天
const VOTING_PERIOD = 5 * 24 * 60 * 60; // 5 天

// 参数级别枚举
const ParameterLevel = {
  APPLICATION: 0,      // 应用级 - 2/3 同意
  SYSTEM: 1,           // 系统级 - 3/4 同意
  INFRASTRUCTURE: 2    // 底层 - 4/5 同意
};
 ...
 */
async function waitUntil(provider, targetTimestamp, maxWaitSeconds = 60) {
  ...
  console.log(`   ⏳ 等待 ${diff} 秒...`);
  await new Promise((resolve) => setTimeout(resolve, diff * 1000));
}

/**
 * 辅助函数：优先尝试快进时间，失败则等待到目标时间
 */
async function advanceTimeOrWait(provider, seconds, targetTimestamp) {
  try {
    await provider.send("evm_increaseTime", [seconds]);
    await provider.send("evm_mine");
    console.log(`   ⏰ 时间快进 ${seconds} 秒`);
  } catch {
    await waitUntil(provider, targetTimestamp);
  }
}
...
async function main() {
  ...
  console.log("   治理代币地址:", await govToken.getAddress());
  ...
  const paramRegistry = await ParameterRegistry.deploy(
    ...
    executor.address  // executor 地址
  );
  await paramRegistry.waitForDeployment();
  console.log("   参数注册合约地址:", await paramRegistry.getAddress());

  // 分发治理代币给投票者
  console.log("\n1.3 分发治理代币...");
  const tokenAmount = ethers.parseEther("100");
  await govToken.mint(storageAdmin.address, ethers.parseEther("50"));
  await govToken.mint(voter1.address, tokenAmount);
  await govToken.mint(voter2.address, tokenAmount);
  await govToken.mint(voter3.address, tokenAmount);
  await govToken.mint(voter4.address, tokenAmount);
  await govToken.mint(voter5.address, tokenAmount);
  console.log("   ✓ 已分发治理代币给各投票者");

  // ============================================================
  // 第二步：注册存储参数
  // ============================================================
  console.log("\n" + "=".repeat(60));
  console.log("第二步：注册存储参数");
  console.log("=".repeat(60));
  ...
  const storageParams = [
    {
      name: "maxFileSize",
      level: ParameterLevel.APPLICATION,
      initialValue: 1024 * 1024 * 100,  // 100MB
      description: "单个文件最大大小限制"
    },
    {
    ...
    {
      name: "shardSize",
      level: ParameterLevel.INFRASTRUCTURE,
      initialValue: 1024 * 1024 * 64,  // 64MB
      description: "分片大小"
    }
  ];
  ...
  for (const param of storageParams) {
    ...
    const registerTx = await paramRegistry.registerParameter(
      paramId,
      param.name,
      param.level,
      CATEGORY_STORAGE,
    ...
    const levelNames = ["APPLICATION", "SYSTEM", "INFRASTRUCTURE"];
    console.log(`   ✓ ${param.name} (${levelNames[param.level]}): ${param.initialValue}`);
  ...
  await advanceTimeOrWait(
    provider,
    VOTING_DELAY + 1,
    Number(proposalDetails.startTime) + 1
  );
  ...
  console.log(`
  1. 部署合约
     - 治理代币: ${await govToken.getAddress()}
     - 参数注册: ${await paramRegistry.getAddress()}

  2. 注册参数
     - 分类: storage
     - 参数: maxFileSize, replicationFactor, shardSize
